...
{  
  ...
  // Shape 1 (Sphere)
  /* G4Material* shape1_mat = nist->FindOrBuildMaterial("TungstenShield");
 ...
 */
  ...
  G4ThreeVector pos1 = G4ThreeVector(0, 6 * cm, 0);

  // Cylinder shape
  G4double innerRadius = 0.0 * cm;  // Inner radius of the cylinder
  G4double outerRadius = 2.54 * cm;  // Outer radius of the cylinder
  G4double hz = 6.5 * cm;          // Half-length along the z-axis
  G4double startAngle = 0.0 * deg;  // Starting angle
  G4double spanningAngle = 360.0 * deg; // Spanning angle

  G4Tubs* solidShape1 = new G4Tubs("Shape1", innerRadius, outerRadius, hz, startAngle, spanningAngle);  // Create cylinder solid
  ...
  rotm->rotateX(90.0 * deg); // Define rotation matrix to rotate the cylinder around the X-axis by 90 degrees
  ...
  // Shape 1 (Rectangle)
  /* G4Material* shape1_mat = nist->FindOrBuildMaterial("G4_Pb");
  G4ThreeVector pos1 = G4ThreeVector(1.5 * cm, 0, 0);

  // Box shape (rectangular)
  G4double box_halfx = 1.50 * cm; // Half-length in x
  G4double box_halfy = 21.50 * cm; // Half-length in y
  G4double box_halfz = 3.75 * cm; // Half-length in z

  G4Box* solidShape1 =
    new G4Box("Shape1",
              box_halfx, box_halfy, box_halfz);
...
/* G4Material* shape1_mat = nist->FindOrBuildMaterial("TungstenShield");
G4ThreeVector pos1 = G4ThreeVector(5.15 * cm, 0, 0);

// Outer box (rectangular)
G4double box_halfx = 4.15 * cm; // Half-length in x
G4double box_halfy = 4.15 * cm; // Half-length in y
G4double box_halfz = 6.25 * cm; // Half-length in z

G4Box* outerBox =
    new G4Box("OuterBox",
              box_halfx, box_halfy, box_halfz);

// Inner box (hollow region)
G4double innerBox_halfx = 2.65 * cm; // Half-length in x for the hollow region
G4double innerBox_halfy = 2.65 * cm; // Half-length in y for the hollow region
G4double innerBox_halfz = 4.75 * cm; // Half-length in z for the hollow region

G4Box* innerBox =
    new G4Box("InnerBox",
              innerBox_halfx, innerBox_halfy, innerBox_halfz);

// Subtract inner box from outer box to create a hollow rectangular shape
G4SubtractionSolid* solidShape1 =
    new G4SubtractionSolid("Shape1", outerBox, innerBox);
...
*/
  ...
  /*  G4Material* shape2_mat = nist->FindOrBuildMaterial("G4_PARAFFIN");
  G4ThreeVector pos2 = G4ThreeVector(0, 0, 0);

  // Outer Sphere shape
  G4double outer_sphere_radius2 = 500.0*cm;  // Adjust the outer radius to fit around the inner sphere
 ...
 */
   ...
   // Shape 2 (Rectangle)
/* G4Material* shape2_mat = nist->FindOrBuildMaterial("G4_PARAFFIN");
G4ThreeVector pos2 = G4ThreeVector(6.0 * cm, 0, 0);

// Box shape (rectangular)
G4double box_halfx2 = 3.0 * cm; // Half-length in x
G4double box_halfy2 = 4.10 * cm; // Half-length in y
G4double box_halfz2 = 4.10 * cm; // Half-length in z

G4Box* solidShape2 =
    new G4Box("Shape2",
              box_halfx2, box_halfy2, box_halfz2);
...
new G4PVPlacement(0,                // no rotation
  ...
  // Shape 2 (Cylinder)
  G4Material* shape2_mat = nist->FindOrBuildMaterial("G4_PARAFFIN");
  G4ThreeVector pos2 = G4ThreeVector(0.00 * cm, 2.5 * cm, 5.94 * cm);

  // Cylinder shape
  G4double innerRadius2 = 0.0 * cm;  // Inner radius of the cylinder
  G4double outerRadius2 = 3.0 * cm;  // Outer radius of the cylinder
  G4double hz2 = 1.8 * cm;           // Half-length along the z-axis
  G4double startAngle2 = 0.0 * deg;  // Starting angle
  G4double spanningAngle2 = 360.0 * deg; // Spanning angle

  G4Tubs* solidShape2 = new G4Tubs("Shape2", innerRadius2, outerRadius2, hz2, startAngle2, spanningAngle2);  // Create cylinder solid
  ...
  /*  G4RotationMatrix* rotm2 = new G4RotationMatrix();
   rotm2->rotateY(90.0 * deg); // Define rotation matrix to rotate the cylinder around the Y-axis by 90 degrees
  */
  new G4PVPlacement(/*rotm2*/0,                // no rotation
		    pos2,             // at position
		    logicShape2,      // its logical volume
		    "Shape2",         // its name
		    logicEnv,         // its mother volume
...
G4ThreeVector pos3 = G4ThreeVector(9.64 * cm, 1 * cm, 0);

// Box shape (rectangular)
G4double box_halfx3 = 0.5 * cm; // Half-length in x
G4double box_halfy3 = 1.4 * cm; // Half-length in y
G4double box_halfz3 = 0.7 * cm; // Half-length in z

G4Box* solidShape3 =
    new G4Box("Shape3",
              box_halfx3, box_halfy3, box_halfz3);
...
new G4PVPlacement(0,                // no rotation
 ...
 G4ThreeVector pos4 = G4ThreeVector(0 * cm, 1 * cm, 11.54 * cm);
 
 // Box shape (rectangular)
 G4double box_halfx4 = 1.4 * cm; // Half-length in x
 G4double box_halfy4 = 1.4 * cm; // Half-length in y
 G4double box_halfz4 = 0.70 * cm; // Half-length in z

 G4Box* solidShape4 =
   new G4Box("Shape4",
	     box_halfx4, box_halfy4, box_halfz4);
 ...
 G4RotationMatrix* rotm4 = new G4RotationMatrix();
   rotm4->rotateY(90.0 * deg);
   rotm4->rotateX(90.0 * deg);
 
 new G4PVPlacement(rotm4,                // no rotation
		   pos4,             // at position
 ...
 /* G4Material* shape3_mat = nist->FindOrBuildMaterial("Eljin");
  ...
  // Outer Sphere shape
  G4double outer_sphere_radius3 = 501.0*cm;  // Adjust the outer radius to fit around the inner sphere
  ...
  G4Sphere* solidShape3 =
    new G4Sphere("Shape3",
		 500.0*cm,  // Inner radius (0 for a full sphere)
