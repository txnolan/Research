...
void B1SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  G4double targetPosition = 5.14 * cm;
  bool Position = (position.mag() > targetPosition);
  // bool Energy = (energy > 5.0 * MeV);

  // Check if the process is neutron capture
  /* const G4String& processName = step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName();
  bool isNeutronCapture = (processName == "G4HadronCaptureProcess");
  */
  
 /* if (Neutron && Position && isNeutronCapture) {
    G4cout << "Neutron capture detected!" << G4endl;
    }*/

 // Detect if the process is neutron capture
  const G4VProcess* postProcess = step->GetPostStepPoint()->GetProcessDefinedStep();
  
  /* bool isNeutronCapture = postProcess && postProcess->GetProcessName() == "nCapture"; */

   bool isNeutronCapture = postProcess->GetProcessName() == "nCapture";

   if (Neutron && isNeutronCapture) {
   ...
   /* if (Neutron && Position && isNeutronCapture) {
     energy = track->GetKineticEnergy();
     fRunAction->IncrementEventCounter();
     track->SetTrackStatus(fStopAndKill);  // Stop the track after capture
     // G4cout << "Neutron capture detected at energy: " << energy/MeV << " MeV" << G4endl;
     } */
